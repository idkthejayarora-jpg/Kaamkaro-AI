...
function AddVendorModal({ onClose, onCreated }: { onClose: () => void; onCreated: (v: Vendor) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-dark-300 border border-dark-50 rounded-2xl w-full max-w-md shadow-2xl animate-slide-up max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 pb-6 flex gap-3 flex-shrink-0">
          ...
          <button type="button" onClick={() => handleSubmit()} disabled={loading} className="btn-primary flex-1">{loading ? 'Adding...' : 'Add Vendor'}</button>
        </div>
      </div>
    </div>
  );
